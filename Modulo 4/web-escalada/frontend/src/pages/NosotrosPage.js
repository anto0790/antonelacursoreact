import React from "react";
import '../styles/NosotrosPage.css';


const NosotrosPage = (props) =>{
    return(
        <selection className="holder">
            <div className="historia">
                <h2>Historia</h2>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores iure quasi cum est expedita laboriosam atque, reiciendis eligendi tempore debitis quia provident incidunt aliquam reprehenderit saepe quis. Atque, provident fugit.
                Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores iure quasi cum est expedita laboriosam atque, reiciendis eligendi tempore debitis quia provident incidunt aliquam reprehenderit saepe quis. Atque, provident fugit.
                </p>
            </div>
            <h2>Staff</h2>
            <div className="personas">
                <div className="persona">
                    <img src="images/nosotros/nosotros1.jpg" width="75" alt="Pablo" />
                    <h5>Pablo T</h5>
                    <h6>Puesto que ocupa</h6>
                    <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores iure quasi cum est expedita laboriosam atque, reiciendis eligendi tempore debitis quia provident incidunt aliquam reprehenderit saepe quis. Atque, provident fugit.</p>
                </div>
                <div className="persona">
                    <img src="images/nosotros/nosotros2.jpg" width="75" alt="Ana" />
                    <h5>Ana R</h5>
                    <h6>Puesto que ocupa</h6>
                    <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores iure quasi cum est expedita laboriosam atque, reiciendis eligendi tempore debitis quia provident incidunt aliquam reprehenderit saepe quis. Atque, provident fugit.</p>
                </div>
                <div className="persona">
                    <img src="images/nosotros/nosotros3.jpg" width="75" alt="Juan" />
                    <h5>Juan G</h5>
                    <h6>Puesto que ocupa</h6>
                    <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores iure quasi cum est expedita laboriosam atque, reiciendis eligendi tempore debitis quia provident incidunt aliquam reprehenderit saepe quis. Atque, provident fugit.</p>
                </div>
                <div className="persona">
                    <img src="images/nosotros/nosotros4.jpg" width="75" alt="Eli" />
                    <h5>Eli M</h5>
                    <h6>Puesto que ocupa</h6>
                    <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores iure quasi cum est expedita laboriosam atque, reiciendis eligendi tempore debitis quia provident incidunt aliquam reprehenderit saepe quis. Atque, provident fugit.</p>
                </div>
                <div className="persona">
                    <img src="images/nosotros/nosotros5.jpg" width="75" alt="Ale" />
                    <h5>Ale S</h5>
                    <h6>Puesto que ocupa</h6>
                    <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Asperiores iure quasi cum est expedita laboriosam atque, reiciendis eligendi tempore debitis quia provident incidunt aliquam reprehenderit saepe quis. Atque, provident fugit.</p>
                </div>
            </div>
        </selection>
    );
}

export default NosotrosPage;